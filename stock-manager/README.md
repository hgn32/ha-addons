...
- **ダッシュボード**: 在庫一覧。カテゴリフィルタ・並び替え（アラート順 / 在庫数 / 名前）。在庫追加・消費・履歴をカード上から操作
...
`node-cron` で定期実行します（既定: `0 18 * * *`（UTC, 毎日 18:00）＝ `0 3 * * *`（JST, 03:00））。  
スケジュールは `amazon_cron` オプション、または `AMAZON_CRON` 環境変数で変更できます（**UTC** で解釈されます）。
...
アドオン専用の設定ディレクトリ（コンテナ内 `/config`、ホスト実体 `/addon_configs/<slug>/`、バックアップ対象）に保存します。
