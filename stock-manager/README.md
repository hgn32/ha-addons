...
- **簡単棚卸し（バーコード）**: 商品のバーコード（JANコード）をカメラ／USBバーコードリーダー／手入力で読み取り、その場で在庫を登録。「数えた数にする（棚卸し）」「数えた数を追加（入庫）」の2モード
- **品目マスタ**: 写真・JANコード・Amazon ASIN・カテゴリ・置き場・購入先・メモを管理。ドラッグ並び替え・CSV出力対応
- **在庫操作**: 在庫追加・在庫消費・強制メンテ（直接数量指定）
  - **入り数 / 実数量の切替**: 在庫追加時に「入り数で指定（×入り数で換算）」と「実数量で指定（個数をそのまま加算）」を選択可能。セット買いでない場合や、使ってから登録する場合に対応
  - **カード履歴からの強制メンテ（棚卸し）**: ダッシュボードのカード履歴ダイアログから、実際に数えた在庫数を入力すると、その数に合わせる調整履歴を登録（履歴は書き換えず1件追加）
