...
| 設定キー | 型 | 既定値 | 説明 |
|---|---|---|---|
| `max_memory_mb` | int | `2048` | VOICEVOX Engine プロセスの仮想メモリ上限（`ulimit -v`、単位 MB） |

VOICEVOX Engine は Python プロセスのため、JVM の `-Xmx` のような正確なヒープ上限指定はできません。
代わりに OS の `ulimit -v`（仮想アドレス空間の上限）で代用しているため、共有ライブラリ等の
mmap 分だけ実際のメモリ使用量（RSS）より緩い上限になります。起動直後に落ちる場合は値を大きくしてください。
