...
代わりに OS の `ulimit -v`（仮想アドレス空間の上限）で代用しています。仮想アドレス空間は
onnxruntime の予約領域などで実際のメモリ使用量（RSS）よりはるかに大きくなるため、
小さい値を設定するとエンジンがメモリ確保に失敗して起動できず、ポート 50021 が
`Connection refused` になります。設定する場合は目安として 8192 以上を推奨します。
エンジンが起動しなくなった場合は `0`（無制限）に戻してください。
