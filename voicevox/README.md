...
**既知の問題（HA 設定 UI）**: `preload_speaker_ids` を GUI のフォームで保存すると、
HA フロントエンドの不具合により `Invalid list for option 'preload_speaker_ids'` エラーで
保存に失敗することがあります。その場合はオプション画面右上の **⋮ → 「YAMLとして編集」** に
切り替えて、上記のようなリスト形式（`- 10` の行形式）で保存してください。

**注意（`max_memory_mb` と併用する場合）**: モデルは 1 スタイル分の読み込みごとに
仮想メモリ（VSZ）が約 0.9GB 増えます。小さめの上限（例 3000MB）で運用していると、
事前読み込みしていないスタイルを初めて使った瞬間に上限を超えて合成が失敗することが
あります。複数スタイルを使う場合は `0`（無制限）にするか、事前読み込みする
スタイル数 × 1GB 程度の余裕を持たせてください。
