...
`GeoLite2-Country.mmdb` はアドオン起動時に `geoipupdate` が自動生成（ダウンロード）します。事前に以下を準備してください。

1. [MaxMind](https://www.maxmind.com/) でアカウントを作成
2. ライセンスキーを発行し、`/share/nginx/GeoIP.conf` に記載する（[サンプル](./sample/GeoIP.conf)参照）
