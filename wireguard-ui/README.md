...
| 種類 | パス(HA から見た場所) |
|---|---|
| クライアント・ユーザー・サーバー設定 DB(JSON ファイル群) | `/config/wireguard-ui/db/` |
| セッション暗号化キー | `/config/wireguard-ui/session-key` |

HA の config フォルダ配下に保存されるため、**アンインストールしても消えず**、
HA のバックアップ対象にも含まれます。File editor や Samba からも閲覧できます。
