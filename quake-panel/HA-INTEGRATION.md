...
- 続報は毎秒のように来るので、**意味が変わったときだけ**流す。鍵は id・警報種別・
  ...
  ので取りこぼさない）
- **表示終了後の焼き直しは上流が捨てる**（docs/eew-events.md §6-1 の
  `isStaleRepeat`）。ブリッジの鍵は `expired` で `none` に戻るので、同じ電文が
  webhook まで来ればそのまま `new` として流れてしまう。上流 `6613fa6` より前は
  実際にそうなっていて、1 回の地震で `new` が 3 回飛んでいた。**ブリッジ側で
  重ねて弾くことはしない**（同じ判定を 2 か所に置くと、上流が直したあとに
  どちらが効いているのか分からなくなる）
