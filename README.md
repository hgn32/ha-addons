...
[Apache Guacamole](https://guacamole.apache.org/) のクライアントレス・リモートデスクトップゲートウェイ（RDP/VNC/SSH/Telnet）です。[alexbelgium 版](https://github.com/alexbelgium/hassio-addons/tree/master/guacamole)を参考に軽量化し、ingress 経由の自動ログイン、設定のみのバックアップ、PostgreSQL 内ログの cron 削除に対応しています。
...
**試験的機能。** 毎朝決まった時刻に最小トークンのタスクを送信し、Claude Pro/Max の5時間セッションを起点にすることを狙うアドオンです。効果があるかは未検証のため、必ず [README](./claude-session-opener/README.md) を読んでから利用してください。
