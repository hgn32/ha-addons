...
`max_memory_mb`（既定 1024）で Tomcat（Guacamole Web）の JVM 最大ヒープサイズ（`-Xmx`）を指定します。
guacd（RDP/VNC/SSH の処理を行うプロセス）は JVM ではないため対象外です。

同時接続数が多い、または動作が重い場合は値を大きくしてください。逆にメモリに余裕のない
環境では値を小さくできますが、小さすぎると Tomcat が `OutOfMemoryError` で落ちることがあります。
